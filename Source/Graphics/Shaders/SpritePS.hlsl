...
float4 PS(GeoOut input) : SV_TARGET
{
	...
	ComputePointLight(gMaterial, gPointLight, input.PosW, input.NormalW, toEye, A, D, S);
	ambient += A;
	diffuse += D;
	specular += S;

	ComputeSpotLight(gMaterial, gSpotLight, input.PosW, input.NormalW, toEye, A, D, S);
	ambient += A;
	diffuse += D;
	specular += S;
